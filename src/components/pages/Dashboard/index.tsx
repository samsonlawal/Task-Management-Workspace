...
function Dashboard() {
  ...
  // Detect current user's role in the active workspace
  const currentUserMember = useMemo(() => {
    if (!user) return null;
    return members.find((m: any) => {
      const userObj = m.userId || m;
      return userObj?._id === user?._id || userObj?.id === user?._id || userObj?.email === user?.email;
    });
  }, [members, user]);

  const userRole = currentUserMember?.role?.toLowerCase() || "member";
  ...
  // Compute active tasks based on viewScope
  const activeTasksList = useMemo(() => {
    if (viewScope === "personal" && user) {
      return tasks.filter((t: any) => t.assignee?.email === user.email);
    }
    return tasks;
  ...
  const completedTasksCount = useMemo(() => {
    return activeTasksList.filter((t: any) => t.status?.toLowerCase() === "done" || t.status?.toLowerCase() === "completed").length;
  }, [activeTasksList]);
  ...
  // Compute workloads per member (Only computed when viewing workspace analytics)
  ...
  const upcomingTasks = useMemo(() => {
    return activeTasksList.filter((t: any) => t.status?.toLowerCase() !== "done" && t.status?.toLowerCase() !== "completed")
      .slice(0, 4);
  }, [activeTasksList]);
  ...
  return (
    <div className="flex h-fit w-full flex-col gap-5 pb-10 transition-all duration-300">
      ...
      <div className="sticky top-0 w-full bg-[white] dark:bg-[#111] z-40 px-4 lg:px-8">
        <div className="poppins flex w-full items-center justify-between border-b border-[#565656]/10 py-[7px]">
          ...
          <div className="flex flex-row items-center justify-center gap-3">
            ...
            {/* View Scope Toggle Selector */}
            <div className="flex items-center bg-gray-100 dark:bg-zinc-800/80 rounded-lg p-0.5 text-[11px] border border-[#565656]/10 mr-1">
              <button
                onClick={() => setViewScope("workspace")}
                disabled={!isAdminOrOwner}
                className={`px-3 py-1.5 rounded-md transition-all duration-200 ${
                  viewScope === "workspace"
                    ? "bg-[#563892] text-white shadow-sm font-semibold"
                    : "text-zinc-500 hover:text-zinc-800 dark:text-zinc-400 dark:hover:text-white disabled:opacity-50 disabled:cursor-not-allowed"
                }`}
                title={!isAdminOrOwner ? "Workspace analytics are Admin-only" : "Workspace Analytics"}
              >
                Workspace
              </button>
              <button
      ...
      {user && !isAdminOrOwner && (
        <div className="mx-4 lg:mx-8 mb-1 p-3 bg-zinc-50/50 dark:bg-zinc-900/30 border border-[#565656]/15 rounded-lg flex items-center justify-between text-[12px] text-zinc-500">
          <div className="flex items-center gap-2">
            <Lock className="w-3.5 h-3.5 text-zinc-400" />
            <span>Viewing <strong>Personal Dashboard</strong>. Workspace-wide metrics are restricted to workspace Admins & Owners.</span>
          </div>
        </div>
      )}
      ...
      <div className="px-4 lg:px-8 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-2">
          ...
          <div className="rounded-[8px] border border-[#565656]/15 bg-white p-5 shadow-sm dark:bg-[#1a1a1a]/30 dark:border-[#565656]/10 hover:border-zinc-400 dark:hover:border-zinc-700 transition-all duration-200">
            <div className="flex items-center justify-between">
              <span className="text-[12px] font-medium tracking-wide text-[#707070] uppercase">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-2">
          ...
          <div className="lg:col-span-2 space-y-5">
            {/* Workload Distribution (Only shown in Workspace Scope) */}
            {viewScope === "workspace" && (
              <div className="rounded-[8px] border border-[#565656]/15 bg-white p-6 shadow-sm dark:bg-[#1a1a1a]/30 dark:border-[#565656]/10 animate-fadeIn">
            ...
            <div className="rounded-[8px] border border-[#565656]/15 bg-white p-6 shadow-sm dark:bg-[#1a1a1a]/30 dark:border-[#565656]/10">
              <div className="flex items-center justify-between border-b border-[#565656]/10 pb-4 mb-4">
                <h3 className="text-sm font-semibold text-[#111] dark:text-white flex items-center gap-2">
                  ...
                  <span>
                    {viewScope === "personal" ? "My Urgent Tasks" : "Urgent Milestones"}
                  </span>
                </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {upcomingTasks.length > 0 ? (
                  upcomingTasks.map((t: any) => {
                    ...
                    // Formats full taskData shape expected by TaskDetails
                    ...
                    return (
                      <div key={t._id || t.id} className="p-3 border border-[#565656]/10 rounded-lg hover:border-zinc-400 dark:hover:border-zinc-700 transition-colors flex flex-col justify-between h-[105px]">
                        ...
                        <h4 className="text-[13px] font-medium text-[#111] dark:text-white mt-2.5 truncate">{t.title}</h4>
                      </div>
                    );
                  })
                ) : (
          ...
          <div className="space-y-5">
            ...
            <div className="rounded-[8px] border border-[#565656]/15 bg-white p-6 shadow-sm dark:bg-[#1a1a1a]/30 dark:border-[#565656]/10">
              <h3 className="text-sm font-semibold text-[#111] dark:text-white flex items-center gap-2 border-b border-[#565656]/10 pb-4 mb-4">
                ...
                <span>
                  {viewScope === "personal" ? "My Activity Log" : "Workspace Activity"}
                </span>
              </h3>
              ...
              <div className="flow-root">
                <ul className="-mb-8">
                  {recentActivities
                    .filter(act => {
                      if (viewScope === "personal" && user) {
                        // In personal dashboard, filter to items related to the user's name
                        return act.user.toLowerCase().includes("samson") || act.type === "integration";
                      }
                      return true;
                    })
                    .map((act, index, arr) => (
