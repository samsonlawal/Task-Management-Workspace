...
const SocialAuthButton = ({
  ...
  return (
    <div className="flex flex-1 self-stretch md:self-start" id={containerId}>
      <button
        title={title}
        className="flex-1 inline-flex items-center justify-center gap-[15px] font-gordita text-[14px] md:text-[16px] leading-6 font-normal text-[#4e4e4e] dark:text-[#E7E7E7] rounded-[10px] border-[1px] border-[#E6E8EC] dark:border-[#E7E7E7] py-[8px] px-[15px]"
        type="button"
        id={buttonId}
        onClick={onClick}
      >
...
const SocialAuthProvidersContent = ({ seterrorMsg }: { seterrorMsg?: any }) => {
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        onGoogleauth({
          payload: userInfo,
          successCallback: () => {
            if (pathname === '/auth/sign-in') router.replace('/user/my-account/my-websites');
          },
...
export const SocialAuthProviders = ({ seterrorMsg }: { seterrorMsg?: any }) => {
  return (
    <GoogleOAuthProvider clientId={process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID as string}>
      <SocialAuthProvidersContent seterrorMsg={seterrorMsg} />
    </GoogleOAuthProvider>
  );
};
